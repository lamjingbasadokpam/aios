...
class AgentRuntime:
    ...
    def __init__(self, model_router: ModelRouter, tool_gateway: ToolGateway,
                 ...
                 recovery_handler: RecoveryHandler | None = None) -> None:
    ...
    def _recover(self, exc: Exception, attempt: int) -> bool:
        if self.recovery_handler is None or classify_failure(exc) is not RecoveryClass.TRANSIENT:
            return False
            ...
            while True:
                try:
                    response = await self.model_router.generate(
                        InferenceRequest(prompt=self._prompt(task, history),
                ...
                if action["action"] == "final":
                    return RuntimeResult(True, output=str(action.get("answer", "")), steps=step, tool_results=tool_results)
                ...
                try:
                    result = await self.tool_gateway.invoke(
                        tool_id, arguments,
                        ToolContext(task_id=task_id, agent_id=agent_id,
                ...
                tool_results.append(result)
                history.append({"step": step, "attempt": attempt, "tool": tool_id,
                                "success": True, "output": result.output, "error": result.error})
                break
        return RuntimeResult(False, error=f"Maximum agent steps exceeded ({self.config.max_steps})",
                             steps=self.config.max_steps, tool_results=tool_results)
